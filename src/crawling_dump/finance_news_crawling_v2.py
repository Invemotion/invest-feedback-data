# ─────────────────────────────────────────────────────────
# finance_news_crawling_v2.py (v2)
# 변경점(대비 v1)
# - 본문 수집 모듈화: fetch_article_content()로 함수 분리(재사용·가독성 ↑).
# - 인코딩 자동 판별: resp.apparent_encoding 사용(목록/기사 페이지 안정성 ↑).
# - 선택자 확장: #articleBodyContents, div#contents div.newsct_body,
#                article#dic_area, div.newsct_body 순차 탐색 + 전체문서 fallback.
# - 로깅 강화: 링크 이동/본문 컨테이너/본문 없음 상세 출력(디버깅 용이).
# - 예외 복원력: try/except 범위 확장 및 빈 본문 안전 처리.
# (저장 포맷·페이지네이션 로직은 v1과 호환)
# ─────────────────────────────────────────────────────────
